    ...
    for (let i = 0; i < leadDataRows.length; i++) {
      ...
      if (currentStatus === STATUS.PENDING) {
        if (emailsSentThisExecution < CONFIG.DAILY_EMAIL_QUOTA) {
          ...
          // Generate prompt first and store in variable
          const initialPrompt = getInitialEmailPrompt(firstName, lastService);
          // Get AI content using the prompt text directly
          const aiContent = getAIEmailContent(initialPrompt);
          if (!aiContent) {
